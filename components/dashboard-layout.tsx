...
const dashboardConfig = {
  navigation: [
    ...
    { name: "Tourism", href: "/tourism", icon: Star },
    { name: "Business Hub", href: "/business", icon: TrendingUp },
    { name: "Sports", href: "/sports", icon: Trophy },
    { name: "Lifestyle", href: "/lifestyle", icon: User },
    { name: "Classifieds", href: "/classifieds", icon: Search },
    { name: "Wyoming Records", href: "/wyoming-records", icon: BookOpen },
    { name: "Native History", href: "/native-history", icon: Mountain },
    { name: "Wyoming Pyramid", href: "/wyoming-pyramid", icon: Mountain },
    { name: "Parks & Recreation", href: "/parks", icon: Mountain },
    { name: "Patents", href: "/patents", icon: BookOpen },
    { name: "Property Connection", href: "/property", icon: Home },
    { name: "OSHA Training", href: "/osha", icon: Settings },
...
export function DashboardLayout({ children, title, description, showSidebar = true }: DashboardLayoutProps) {
  ...
  useEffect(() => {
    const adTimer = setInterval(() => {
      setCurrentAdIndex((prev) => (prev + 1) % dashboardConfig.gameAds.length)
    }, 8000)
  ...
  useEffect(() => {
    const posterTimer = setInterval(() => {
      setCurrentPosterIndex((prev) => (prev + 1) % dashboardConfig.wantedPosters.length)
    }, 6000)
  ...
  const currentAd = dashboardConfig.gameAds[currentAdIndex]
  const currentPoster = dashboardConfig.wantedPosters[currentPosterIndex]
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100">
        <div className="container mx-auto px-4 py-8">
          {title && (
            <div className="mb-8">
              <h1 className="text-4xl font-bold text-amber-900 mb-2">{title}</h1>
              {description && <p className="text-lg text-amber-700">{description}</p>}
            </div>
          )}
          {children}
        </div>
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-amber-50 to-orange-100">
      ...
      <div
        className={`
        fixed inset-y-0 left-0 z-50 w-80 bg-gradient-to-b from-amber-900 to-amber-800 
        transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0
        ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}
      `}
      >
        <div className="flex items-center justify-between h-16 px-6 bg-amber-950">
            ...
            {dashboardConfig.navigation.map((item) => (
              ...
              >
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {item.external && <ExternalLink className="h-4 w-4 text-amber-500" />}
                </div>
              </a>
            ))}
          </nav>
            ...
            <div className="flex items-center space-x-4">
              ...
              {title && (
                <div>
                  ...
                  {description && <p className="text-sm text-amber-700">{description}</p>}
                </div>
              )}
            </div>
            <div className="flex items-center space-x-4">
